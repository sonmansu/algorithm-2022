...
void op(Line inputLine, double t) {
    ...
    if (leftDist < midDist &&(midDist - leftDist)>gap && (t- ceil((t+leftNum)/2 * divide) / divide) > gap) { // 1/2
        rightNum = t;
        ...
        if (rightDist < midDist && (midDist - rightDist)>gap && (ceil((t + rightNum) / 2 * divide)/divide - t > gap)) { // 1/2
            leftNum = t;
